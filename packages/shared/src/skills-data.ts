...
import type { Skill, SkillTriggerConfig, IntentType } from './types.js';

/**
 * 额外的触发词映射（用于增强匹配）- 支持中英文
 * 完整数据在 packages/mcp-server/src/skills-controller.ts 的 EXTRA_TRIGGERS
 */
export const EXTRA_TRIGGERS: Record<string, { zh?: string[]; en?: string[] }> = {
 ...
 */
export const SKILL_CONFIGS: Record<string, SkillTriggerConfig> = {
  // 示例数据 - 完整数据需要从 MCP 服务迁移
  "frontend-design": {
    triggers: [
      { word: "设计", weight: 3 },
 ...
 * 从 EXTRA_TRIGGERS 生成
 */
export const SKILLS_REGISTRY: Skill[] = Object.entries(EXTRA_TRIGGERS).map(([name, triggers]) => {
